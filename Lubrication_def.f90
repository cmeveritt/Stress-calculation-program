Subroutine Lubrication_def(HM0r1,  Z1, EDA01, alpha1)
    ...
    ! Common variables for almost all ref
    read (7,*)
    read (7,*)
    read (7,*) Ta ,Z, EDA0, Pref, alpha, RA1, RA2
    alpha=alpha*1E-8
    Pref=Pref*1E8
    RA1=RA1*1E-9             
    RA2=RA2*1E-9
        
    IF( Lub_param .EQ. 1 .or. Lub_param .EQ. 6) THEN
        ...
        Ntime=NX/F
                                            ! Venner gives in both dimension and dimensionless numbers
    ...
    ELSEIF( Lub_param .EQ. 2 ) THEN
        ! Gohars formulation of equations -----------------------------------------------------------------------
    ...
    ELSE IF( Lub_param .EQ. 3)THEN
        ...
        Ntime=Um*NX/(F*Ua)                                          ! dT=F*um/um*dX
        ...
        read (7,*)
        read (7,*)
        read (7,*)  !This is for case 5        
        read (7,*)
        read (7,*)
        read (7,*)  !This is for case 5
        
     ELSE IF( Lub_param .EQ. 4)THEN
         ! R.Larssons formulation of equations ----------------------------------------------------------------------
        ...
        !temperatures
        RL_T0= 40
        RL_Ta=Ta        ! To be on the safe side if used later
        
        Z=Dz+Cz*log10(1+Ta/135)
        S0=-S0
        EDA0 = 10**(-4.2+RL_G0*(1+RL_Ta/135)**S0)             ! Eq (3)!  EDA0 is updated in the newtonian subroutine,  EDA0=10**(-4.2+RL_G0*(1+Ta/135)**S0)           ! Eq (3)
        ...
        read (7,*)
        read (7,*)
        read (7,*)  Temp_param, p_red, temp_red, temp_fac, tau_lim, tau_gamma, lim_factor     ! Different meaning of thmp_red and temp_fac depending on temp param ==1 or == 2.    
        read (7,*)
        read (7,*)
        read (7,*)  !This is for case 5
...
ELSE IF( Lub_param .EQ. 8)THEN
    ...
    ! Hartinger 2008 formulation of equations ----------------------------------------------------------------------
        ...
        Lub_param=9         ! Becouse same equations
    ...
    ELSEIF( shear_thin .EQ. 1) Then
        ! NonNewtonian acc P.Eheret et all
        ...
        read (7,*)
        read (7,*)
        read (7,*)  tauc_real, tauS, taua2, xilim, xi_param
    ...
    ELSE IF( shear_thin .EQ. 4)THEN
        ...
        read (7,*)
        read (7,*)
        read (7,*)  L_n, L_G, L_h_limit, L_iter, L_stab,shear_max,shear_min, temp_max
        L_G=L_g*1e4
        L_h_limit=L_h_limit*1e-9
    ...
    ! To enable passing back to main
    Z1      = Z
    EDA01   = EDA0
    alpha1  = alpha
    HM0r    = HM0r1
